...
def createModel4Training(training_data, training_data_y, validation_data, validation_data_y, cnn_model_json, cnn_weights_h5, kfold_no):
    ...
    # Layer 1
    alexnet.add(Conv2D(96, (11, 11), input_shape=(first_value, second_value, 1), padding='same', kernel_regularizer=l2(l2_reg)))
    ...
    # PLOT
    #keras.utils.plot_model(alexnet, "net.png", show_shapes=True)
    # COMPILE
    alexnet.compile(loss='sparse_categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    K.set_value(alexnet.optimizer.learning_rate, 0.001)

    data_train = training_data.reshape(-1, first_value, second_value, 1)
    data_val = validation_data.reshape(-1, first_value, second_value, 1)


    # CREATE AND SAVE MODEL
    history = alexnet.fit(
        {"conv2d_input": data_train},
        training_data_y,
        batch_size=8,
        validation_data=({"conv2d_input": data_val}, validation_data_y),
        epochs=30, verbose=1,
    ...
    alexnet.save_weights(cnn_weights_h5)

    # Testeo
    scores = alexnet.evaluate(data_val, validation_data_y, verbose=1)
    ...
    plt.savefig('./Graphs/ALEXNET/ACC/Acc_KFold' + str(kfold_no) + '.png', dpi=300)
    plt.close()
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'val'], loc='upper left')
    plt.savefig('./Graphs/ALEXNET/LOSS/Loss_KFold' + str(kfold_no) + '.png', dpi=300)
    plt.close()
...
def createModel4Testing(testing_data, testing_data_y, cnn_model_json, cnn_weights_h5, kfold_no):
    ...
    testing_data = testing_data.reshape(-1, first_value, second_value, 1)
    out = loaded_model.predict(testing_data)
    test_eval = loaded_model.evaluate(testing_data, testing_data_y, verbose=1)
    ...
    plt.legend(loc='best')
    plt.savefig('./Graphs/ALEXNET/RoC/Roc_KFold' + str(kfold_no) + '.png', dpi=300)
    plt.close()
